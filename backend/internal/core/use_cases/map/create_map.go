...
func NewCreateMapUseCase(mapRepository repositories.MapRepository, locationRepository repositories.LocationRepository) *CreateMapUseCase {
...
func (uc *CreateMapUseCase) Execute(input CreateMapInput) (CreateMapOutput, error) {
	...
	existingMap, err := uc.mapRepository.FindByName(input.Name)
	if err != nil {
	...
	if err := uc.mapRepository.Create(newMap); err != nil {
		return CreateMapOutput{}, err
	}
